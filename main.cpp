...
void saveImage() {
	...
	stbi_write_png(fn.c_str(), SCREEN_WIDTH, SCREEN_HEIGHT, 4, buffer,
	               SCREEN_WIDTH * 4);
}
...
void display() {
	...
	*/
	int comp = NWORLDS == 0 ? 30 : NWORLDS;
	if (PROFILING && fc >= comp) {
	...
	if (NWORLDS > 0) {
		cubes->draw(dc % cubes->NUMWORLDS); // PRECOMP ENABLED --> PASS INDEX OF STATE
		dc += animation;     
	} else {      		// UPDATE DC WITH ANIMATION (1) IF ENABLED
		cubes->draw(0); // ELSE WE RENDER REAL TIME
		if(animation) cubes->update();
	};
