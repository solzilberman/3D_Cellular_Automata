...
public:
  ...
  void init(string vertex_shader, string fragment_shader, bool LIGHTING_ENABLED,
    ...
    for (int y = 5; y < side_length - 5; y++) {
      ...
      cells_vec[(int)curr] = lifecycle;
      btranslations.push_back(((unsigned int)(side_length / 2) | (y << 8) |
    ...
    for (int x = 5; x < side_length - 5; x++) {
      float curr = x + side_length * (side_length / 2.0) +
                   side_length * side_length * (side_length / 2.0);
      cells_vec[(int)curr] = lifecycle;
      btranslations.push_back((x | (unsigned int)(side_length / 2) << 8 |
                               (unsigned int)(side_length / 2) << 16));
    }
    ...
    for (int z = 5; z < side_length - 5; z++) {
      ...
      cells_vec[(int)curr] = lifecycle;
      btranslations.push_back(((unsigned int)(side_length / 2) |
    ...
    // vb->use();
    // glBufferSubData(GL_ARRAY_BUFFER, 0, btranslations.size() *
    // sizeof(unsigned int), btranslations.data());
  ...
  void update() {
    ...
    if (REFLECT_DIVISOR > 1) {
      for (int t = 0; t < btranslations.size(); t++) {
  ...
  void draw() {
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0,
                    btranslations.size() * sizeof(unsigned int),
                    btranslations.data());
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    glDrawArraysInstanced(GL_TRIANGLES, 0, 3 * 12, btranslations.size());
  }
